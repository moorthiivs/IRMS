...
export function Reports() {
  ...
  const [historyHasMc, setHistoryHasMc] = useState<boolean>(
    searchParams.get("hasMc") === "true",
  );
  ...
  const { data: recent = [], isLoading: isRecentLoading } = useQuery({
    ...
    queryFn: () => {
      const formattedDate = historyDate
        ? `${historyDate.getFullYear()}-${String(
  ...
  const filteredHistoryParts = historyCustomer
    ? parts.filter((p: any) => p.customerId === historyCustomer)
    : parts;
  ...
  const filteredReportParts = (
    reportCustomer
      ? parts.filter((p: any) => p.customerId === reportCustomer)
      : parts
  ).filter((p: any) =>
    dailyOptions?.partIds ? dailyOptions.partIds.includes(p.id) : false,
  );

  const reportCustomerData = customers.find(
    (c: any) => c.id === reportCustomer,
  );

  // Use dailyOptions mcNos if available and filtering, otherwise fallback to customer machines
  const reportCustomerMachines = dailyOptions?.mcNos
    ? dailyOptions.mcNos
    : reportCustomerData?.machines || [];
  ...
  const { data: operations = [] } = useQuery({
    ...
    select: (data) =>
      data.filter(
        (o) =>
          !dailyOptions?.operationIds ||
          dailyOptions.operationIds.includes(o.id),
      ),
  });
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      await inspectionService.downloadDailyPdf(
        ...
        `Inspector_Inprocess_Check_Sheet_${partStr}_${
  ...
  const handleDelete = (id: string) => {
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Are you sure you want to delete this inspection report? This action
          cannot be undone.
        </Text>
      ),
  ...
  const correctionMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      notifications.show({
        ...
        message:
          "Failed parameters have been corrected. Status has been re-evaluated.",
        color: "green",
      });
    },
    onError: (err: any) => {
      notifications.show({
        title: "Error",
        message: err?.response?.data?.message || "Failed to save corrections.",
        color: "red",
      });
    },
  });
  ...
  const handleSubmitCorrections = () => {
    ...
    const corrections = Object.entries(correctionValues)
      .filter(([detailId, value]) => {
        const original = correctionTx.details?.find(
          (d: any) => d.id === detailId,
        );
    ...
    if (corrections.length === 0) {
      notifications.show({
        title: "No Changes",
        message: "Please modify at least one value to submit a correction.",
        color: "orange",
      });
      return;
    }
  ...
  const handleBulkDelete = () => {
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Are you sure you want to delete {selectedIds.length} inspection(s)?
        </Text>
      ),
      ...
      onConfirm: async () => {
        ...
        try {
          await Promise.all(
            selectedIds.map((id) => inspectionService.deleteInspection(id)),
          );
          notifications.show({
            title: "Deleted",
            message: `${selectedIds.length} report(s) deleted.`,
            color: "green",
          });
          setSelectedIds([]);
          queryClient.invalidateQueries({ queryKey: ["recent-inspections"] });
  ...
  const handleBulkApprove = (groupTxs: any[]) => {
    const unapproved = groupTxs.filter(
      (tx) => tx.status === "PASSED" && !tx.approvedById,
    );
    ...
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Are you sure you want to approve {unapproved.length} pending
          inspection(s) for this group?
        </Text>
      ),
      ...
      onConfirm: async () => {
        ...
        try {
          await Promise.all(
            unapproved.map((tx) => inspectionService.approveInspection(tx.id)),
          );
          notifications.show({
            title: "Approved",
            message: `${unapproved.length} report(s) approved.`,
            color: "green",
          });
          queryClient.invalidateQueries({ queryKey: ["recent-inspections"] });
          queryClient.invalidateQueries({ queryKey: ["daily-report"] });
          ...
          notifications.show({
            title: "Error",
            message: "Failed to approve some reports.",
            color: "red",
          });
        } finally {
  ...
  const getCellContent = (
    ...
    const tx = dailyReportTransactions.find(
      (t) =>
        t.shift?.name?.toLowerCase() === shiftName.toLowerCase() &&
    ...
    if (readings.length >= 2) {
      return (
        <div className="relative h-12 w-full">
          ...
          <span
            className={`absolute top-1 left-1.5 text-[10px] leading-none ${
              readings[0].status === "FAIL"
    ...
    return (
      <div
        className={`flex items-center justify-center h-12 w-full text-center text-xs font-semibold ${
          singleReading.status === "FAIL" ? "text-red-600" : "text-gray-800"
        }`}
      >
  ...
  const isOncePerShift = (param: any) => {
    ...
    const freq = String(param.freqOfInspn || "")
      .toLowerCase()
      .trim();
    return (
      freq === "1" ||
      ...
      return (
        <Table.Td p={0} colSpan={6}>
          <div className="relative h-12 w-full flex items-center justify-center">
            ...
            <span
              className={`absolute top-2 left-[30%] text-xs leading-none ${
                readings[0]?.status === "FAIL"
                  ? "text-red-600 font-bold"
                  : "text-gray-800 font-semibold"
              }`}
            ...
            <span
              className={`absolute bottom-2 right-[30%] text-xs leading-none ${
                readings[1]?.status === "FAIL"
                  ? "text-red-600 font-bold"
                  : "text-gray-800 font-semibold"
              }`}
        ...
        if (tx.details) {
          tx.details.forEach((d: any) => {
            if (d.parameterId === param.id) {
              const shiftIdx = shifts.findIndex(
                (s: any) => s.id === tx.shiftId,
              );
        ...
        <>
          {shiftReadings.map((reading, idx) => (
            <Table.Td
              ...
              p={0}
              colSpan={2}
              className="text-center h-12 align-middle"
            >
              {reading && (
                <span
                  className={`text-xs ${
    ...
    return (
      <Table.Td p={0} colSpan={6} className="text-center h-12 align-middle">
        {readings[readings.length - 1] && (
          <span
            className={`text-xs ${
              readings[readings.length - 1].status === "FAIL"
                ? "text-red-600 font-bold"
                : "text-gray-800 font-semibold"
            }`}
          >
  ...
  const getMergedCellContent = (shiftName: string, param: any) => {
    ...
    return (
      <div
        className={`flex items-center justify-center h-12 w-full text-center text-xs font-semibold ${
          r.status === "FAIL" ? "text-red-600" : "text-gray-800"
        }`}
      >
        {r.observedValue}
      </div>
  ...
  const getTxMetadata = (shiftName: string, intervalName: string) => {
    return dailyReportTransactions.find(
      (t) =>
        t.shift?.name?.toLowerCase() === shiftName.toLowerCase() &&
        t.intervalName === intervalName,
    );
  ...
  const getFooterStatus = (shiftName: string, intervalName: string) => {
    ...
    return (
      <Badge
        color={tx.status === "PASSED" ? "green" : "red"}
        variant="filled"
        size="xs"
      >
  ...
  const getRemarksText = () => {
    ...
    return activeRemarks.length > 0
      ? activeRemarks.join(" | ")
      : "No remarks logged today.";
  };
  ...
  const groupedRecent = useMemo(() => {
    ...
    recent.forEach((tx: any) => {
      const custName = (
        tx.part?.customer?.name || "Unknown Customer"
      ).toUpperCase();
      ...
      if (!groups[l1].children[l2])
        groups[l1].children[l2] = { txs: [], children: {} };
      groups[l1].children[l2].txs.push(tx);

      if (!groups[l1].children[l2].children[l3])
        groups[l1].children[l2].children[l3] = { txs: [] };
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh" }}>
          ...
          <Group justify="space-between" align="center" mb="lg">
            <Group gap="sm" wrap="wrap">
              ...
              <DatePickerInput
                ...
                w={130}
                styles={{
                  input: { border: "none", backgroundColor: "#f1f3f5" },
                }}
              />
              <Select
                placeholder="Shift"
              ...
              <Select
                placeholder="Part"
                size="xs"
                data={filteredHistoryParts.map((p: any) => ({
                  value: p.id,
                  label: p.partNumber,
                }))}
                value={historyPart}
              ...
              <Select
                ...
                size="xs"
                data={historyOperations.map((o) => ({
                  value: o.id,
                  label: o.operationNumber,
                }))}
                value={historyOp}
            ...
            <Group gap="sm">
              {filterStatus && (
                <Badge
                  color={filterStatus === "PASSED" ? "green" : "red"}
                  variant="light"
                >
                  {filterStatus}
                </Badge>
              ...
              {canApprove &&
                ...
                recent.filter(
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleBulkApprove(
                        recent.filter((t: any) => selectedIds.includes(t.id)),
                      )
                    }
                  >
                    Approve (
            ...
            {isRecentLoading ? (
              ...
              <div className="overflow-x-auto">
                <Table
                  ...
                  striped={false}
                  withRowBorders
                  className="min-w-[900px] border border-gray-200 rounded-md overflow-hidden"
                >
                    ...
                    <Table.Tr>
                      <Table.Th style={{ width: 40, borderBottom: "none" }}>
                        <Checkbox
                          checked={
                            recent.length > 0 &&
                            selectedIds.length === recent.length
                          }
                          indeterminate={
                            selectedIds.length > 0 &&
                            selectedIds.length < recent.length
                      ...
                      <Table.Th style={{ borderBottom: "none", width: 140 }}>
                        <Text size="xs" fw={600} c="dimmed">
                          Assigned On
                        </Text>
                      </Table.Th>
                  ...
                  <Table.Tbody>
                    {Object.entries(groupedRecent).map(
                      ([l1Key, l1Node]: [string, any]) => (
                        <React.Fragment key={l1Key}>
                              ...
                              <Group justify="space-between">
                                <Group gap="sm">
                                  ...
                                  <Checkbox
                                    ...
                                    checked={
                                      l1Node.txs.length > 0 &&
                                      l1Node.txs.every((t: any) =>
                                        selectedIds.includes(t.id),
                                      )
                                    }
                                    indeterminate={
                                      l1Node.txs.some((t: any) =>
                                        selectedIds.includes(t.id),
                                      ) &&
                                      !l1Node.txs.every((t: any) =>
                                        selectedIds.includes(t.id),
                          ...
                          {expandedGroups[l1Key] &&
                            Object.entries(l1Node.children).map(
                              ([l2Key, l2Node]: [string, any]) => {
                                ...
                                return (
                                  <React.Fragment key={l2FullKey}>
                                        ...
                                        <Group justify="space-between" ml={32}>
                                          <Group gap="sm">
                                            ...
                                            <Checkbox
                                              ...
                                              checked={
                                                l2Node.txs.length > 0 &&
                                                l2Node.txs.every((t: any) =>
                                                  selectedIds.includes(t.id),
                                                )
                                              }
                                              indeterminate={
                                                l2Node.txs.some((t: any) =>
                                                  selectedIds.includes(t.id),
                                                ) &&
                                                !l2Node.txs.every((t: any) =>
                                                  selectedIds.includes(t.id),
                                    ...
                                    {expandedGroups[l2FullKey] &&
                                      Object.entries(l2Node.children).map(
                                        ([l3Key, l3Node]: [string, any]) => {
                                          ...
                                          return (
                                            <React.Fragment key={l3FullKey}>
                                              ...
                                              {/* LEVEL 4: Items */}
                                              {expandedGroups[l3FullKey] &&
                                                l3Node.txs.map((item: any) => (
                                                  <Table.Tr
                                                    key={item.id}
          ...
          <Paper withBorder p="md" radius="md" mb="lg">
            <SimpleGrid cols={{ base: 1, sm: 2, lg: 6 }} spacing="md">
              ...
              <Select
                ...
                data={filteredReportParts.map((p: any) => ({
                  value: p.id,
                  label: p.partNumber,
                }))}
                value={selectedPart}
              ...
              <Select
                ...
                data={
                  reportCustomerMachines.length > 0
                    ? reportCustomerMachines
                    : selectedMcNo
                    ? [selectedMcNo]
                    : []
                }
      ...
      >
        <Text size="sm" mb="md">
          Are you sure you want to approve this inspection? You must read and
          verify the values before approving.
        </Text>
        <Checkbox
          ...
          <div>
            ...
            >
              <Group gap="xs" mb="xs">
                ...
                <Text size="sm" fw={600}>
                  {correctionTx.part?.partNumber} /{" "}
                  {correctionTx.operation?.operationNumber}
                </Text>
              </Group>
              <Text size="xs" c="dimmed">
                {correctionTx.shift?.name} — {correctionTx.intervalName} —{" "}
                {new Date(correctionTx.inspectionTimestamp).toLocaleString()}
              </Text>
            </Paper>

            <Text size="sm" fw={600} mb="sm">
              Failed Parameters (editable):
            </Text>
            ...
            <Table withTableBorder withColumnBorders verticalSpacing="sm">
              ...
              <Table.Tbody>
                {correctionTx.details
                  ?.filter((d: any) => d.status === "FAIL")
                  .map((detail: any) => (
                    <Table.Tr key={detail.id}>
                      <Table.Td>
                        <Text size="sm" fw={500}>
                          {detail.parameter?.parameterName}
                        </Text>
                      </Table.Td>
                      <Table.Td>
                        <Text size="xs">
                          {detail.parameter?.specText || "-"}
                        </Text>
                      </Table.Td>
                      <Table.Td>
                        ...
                        {(() => {
                          const getCorrectionStatus = (
                            ...
                            if (
                              ...
                              detail.parameter?.controlLimitMax !== null
                            ) {
                              ...
                              if (
                                numVal <
                                  detail.parameter.controlLimitMin - EPSILON ||
                                numVal >
                                  detail.parameter.controlLimitMax + EPSILON
                              )
                                return "FAIL";
                              return "PASS";
                            } else {
                              ...
                              if (lower === "ng" || lower === "fail")
                                return "FAIL";
                              if (
                                /\b(ng|fail|reject|rejected|not ok)\b/.test(
                                  lower,
                                )
                              )
                                return "FAIL";
                              ...
                              <Box style={{ flex: 1 }}>
                                {detail.parameter?.controlLimitMin !== null ? (
                                  <TextInput
                                    ...
                                    inputMode="decimal"
                                    step={
                                      detail.parameter?.leastCount &&
                                      detail.parameter.leastCount > 0
                                        ? String(detail.parameter.leastCount)
                                        : "0.001"
                                    }
                                  />
                                ) : (
        ...
        {auditTrailData.length === 0 ? (
          <Paper withBorder p="xl" radius="md">
            <Text ta="center" c="dimmed">
              No corrections have been made to this inspection.
            </Text>
          </Paper>
        ) : (
          <Timeline
            active={auditTrailData.length - 1}
            bulletSize={28}
            lineWidth={2}
          >
            {auditTrailData.map((entry) => (
              <Timeline.Item
                ...
                bullet={
                  <ThemeIcon
                    ...
                    variant="filled"
                    color={
                      entry.correctedStatus === "PASS" ? "green" : "orange"
                    }
                    radius="xl"
                  >
                    <Wrench size={14} />
                ...
                title={
                  <Group gap="xs">
                    <Text size="sm" fw={600}>
                      {entry.detail?.parameter?.parameterName || "Parameter"}
                    </Text>
                    <Badge
              ...
              >
                ...
                <Text size="xs" c="dimmed" mt={4}>
                  By {entry.correctedBy?.name || "Unknown"} at{" "}
      ...
      {activeTab === "daily" && dailyReportTransactions.length > 0 && (
        <div className="mt-2 print:mt-0">
            ...
            <Button
              ...
              onClick={() => {
                const formattedDate = selectedDate
                  ...
                  : "";
                window.open(
                  `/reports/daily/preview?partId=${selectedPart}&opId=${selectedOp}&mcNo=${selectedMcNo}&date=${formattedDate}`,
                  "_blank"
                );
              }}
            >
          ...
          <Paper
            withBorder
            p="xl"
            className="print:border-0 print:p-0 bg-white shadow-sm overflow-x-auto print:overflow-visible text-black"
          >
            <div
              ref={reportRef}
              id="daily-report-print-container"
              className="min-w-[900px] print:min-w-full p-4 bg-white text-black"
            >
              {/* Header Box matching sample.pdf PDF format (100% Aligned Grid Connected Border) */}
              <div
                className="report-header-box flex items-stretch text-sm font-medium bg-white text-black"
                style={{ border: "1px solid #000", borderBottom: "none" }}
              >
                {/* Left section: Logo + Company info + Date / Machine info */}
                <div
                  className="w-[34.5%] flex flex-col justify-between"
                ...
                >
                  <div className="p-2.5 flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-extrabold text-[13px] text-[#000080] uppercase tracking-tight truncate">
                        {settings.report_company_name !== undefined
                      ...
                      {(settings.report_company_subtitle || (!settings.report_company_name && settings.report_company_subtitle !== "")) && (
                        <div className="text-[9.5px] text-[#000080] font-semibold leading-tight mt-0.5 whitespace-pre-line">
                          {settings.report_company_subtitle ?? ""}
                        </div>
                      )}
                    </div>
                  </div>
                  <div
                    className="px-2.5 py-1.5 text-xs flex justify-between"
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="font-bold pl-1">
                        {selectedMcNo ||
                          dailyReportTransactions[0]?.mcNo ||
                          "N/A"}
                      </span>
                    </div>
                  </div>
                ...
                <div
                  className="w-[36.5%] p-2 flex flex-col justify-center items-center text-center"
                  style={{ borderRight: "1px solid #000" }}
                >
                  <Text
                    size="md"
                    fw={900}
                    className="text-black uppercase font-black tracking-wider leading-tight"
                  >
                    {settings.inspection_report_title ||
                      "INSPECTOR - INPROCESS CHECK SHEET"}
                  </Text>
                </div>
                  ...
                  >
                    <tbody>
                      <tr style={{ borderBottom: "1px solid #000" }}>
                        <td
                          className="font-bold w-[42%] p-1.5 bg-gray-50 text-[10px]"
                          style={{ borderRight: "1px solid #000" }}
                        >
                          PART NO.
                        </td>
                        <td className="font-bold p-1.5 text-xs">
                          {currentPartInfo?.partNumber || "N/A"}
                        </td>
                      </tr>
                      <tr style={{ borderBottom: "1px solid #000" }}>
                        <td
                          className="font-bold p-1.5 bg-gray-50 text-[10px]"
                          style={{ borderRight: "1px solid #000" }}
                        >
                          PART NAME
                        </td>
                        <td className="font-bold p-1.5 text-[11px] truncate">
                          {currentPartInfo?.partName || "N/A"}
                        </td>
                      </tr>
                      <tr>
                        <td
              ...
              >
                <Table.Thead className="bg-gray-50 dark:bg-[#25262b] text-center font-bold text-xs">
                  <Table.Tr>
                    ...
                    <Table.Th
                      colSpan={2}
                      className="text-center bg-blue-50 dark:bg-blue-900/20 text-black dark:text-white"
                    >
                      1st Shift (Shift A)
                    </Table.Th>
                    <Table.Th
                      colSpan={2}
                      className="text-center bg-teal-50 dark:bg-teal-900/20 text-black dark:text-white"
                    >
                      2nd Shift (Shift B)
                    </Table.Th>
                    <Table.Th
                      colSpan={2}
                      className="text-center bg-orange-50 dark:bg-orange-900/20 text-black dark:text-white"
                    >
                      3rd Shift (Shift C)
                    </Table.Th>
                  </Table.Tr>
                  <Table.Tr>
                    <Table.Th
                      style={{ width: 85 }}
                      className="text-center bg-blue-50/50 dark:bg-blue-900/10"
                    >
                      1 Half
                    </Table.Th>
                    <Table.Th
                    ...
                    <Table.Th
                      style={{ width: 85 }}
                      className="text-center bg-teal-50/50 dark:bg-teal-900/10"
                    >
                      1 Half
                    </Table.Th>
                    <Table.Th
                    ...
                    <Table.Th
                      style={{ width: 85 }}
                      className="text-center bg-orange-50/50 dark:bg-orange-900/10"
                    >
                      1 Half
                    </Table.Th>
                    <Table.Th
                ...
                <Table.Tbody className="text-xs">
                    ...
                    return (
                      ...
                      >
                        ...
                        <Table.Td className="text-center font-medium">
                          {param.specText ||
                            `${param.nominalValue} ±${param.upperTolerance}`}
                        </Table.Td>

                        <Table.Td className="text-center">
                          {param.methodOfChecking || "-"}
                        </Table.Td>
                        <Table.Td className="text-center font-medium">
                          {param.freqOfInspn || "-"}
                        </Table.Td>
                        ...
                        {isOncePerDay(param) ? (
                          ...
                          <>
                            <Table.Td p={0} colSpan={2}>
                              {getMergedCellContent("Shift A", param)}
                            </Table.Td>
                            <Table.Td p={0} colSpan={2}>
                              {getMergedCellContent("Shift B", param)}
                            </Table.Td>
                            <Table.Td p={0} colSpan={2}>
                              {getMergedCellContent("Shift C", param)}
                            </Table.Td>
                          </>
                        ) : (
                          <>
                            <Table.Td p={0}>
                              {getCellContent("Shift A", "1 Half", param)}
                            </Table.Td>
                            <Table.Td p={0}>
                              {getCellContent("Shift A", "2 Half", param)}
                            </Table.Td>
                            <Table.Td p={0}>
                              {getCellContent("Shift B", "1 Half", param)}
                            </Table.Td>
                            <Table.Td p={0}>
                              {getCellContent("Shift B", "2 Half", param)}
                            </Table.Td>
                            <Table.Td p={0}>
                              {getCellContent("Shift C", "1 Half", param)}
                            </Table.Td>
                            <Table.Td p={0}>
                              {getCellContent("Shift C", "2 Half", param)}
                            </Table.Td>
                          </>
                        )}
                      </Table.Tr>
                  ...
                  <Table.Tr className="bg-gray-50/80 dark:bg-[#25262b]/80 font-bold text-center">
                  ...
                  <Table.Tr>
                    ...
                    <Table.Td colSpan={2} className="font-medium">
                      No Production
                    </Table.Td>
                    <Table.Td
                      colSpan={2}
                      rowSpan={3}
                      className="align-top italic p-2 text-xs"
                    >
                      {getRemarksText()}
                    </Table.Td>
                  ...
                  <Table.Tr>
                    ...
                    <Table.Td colSpan={2} className="text-center text-[10px]">
                      {getApproverSignature("Shift A")}
                    </Table.Td>
                    <Table.Td colSpan={2} className="text-center text-[10px]">
                      {getApproverSignature("Shift B")}
                    </Table.Td>
                    <Table.Td colSpan={2} className="text-center text-[10px]">
                      {getApproverSignature("Shift C")}
                    </Table.Td>
                  </Table.Tr>
                  <Table.Tr>
                    <Table.Td
                      colSpan={3}
                      className="text-center text-[9px] text-gray-500 font-medium"
                    >
                      {settings.inspection_report_doc_number ||
                        "TAF / P2 / 9.1B"}{" "}
                      (Rev date:{" "}
                      {settings.inspection_report_r_date || "06.10.2023"})
                    </Table.Td>
                    <Table.Td className="text-center font-bold whitespace-nowrap text-[11px] p-1">
                      Approved by
                    </Table.Td>
                  ...
                  <Table.Tr className="bg-gray-50/50 dark:bg-[#25262b]/50 font-medium print:hidden">
                    <Table.Td colSpan={6} className="text-right pr-4">
                      Approve Action
                    </Table.Td>
                      ...
                      return (
                        ...
                        >
                          {canApprove && unapproved.length > 0 ? (
                            <Tooltip
                              label={`Approve ${unapproved.length} transaction(s) for ${shift}`}
                            >
                              <Button
                                size="compact-xs"
                                color="yellow"
                                  ...
                                  modals.openConfirmModal({
                                    ...
                                    children: (
                                      <Text size="sm">
                                        ...
                                        report for {shift}? Your signature will
                            ...
                            dailyReportTransactions.some(
                              (t) => t.shift?.name === shift,
                            ) ? (
      ...
      {activeTab === "daily" && dailyReportTransactions.length === 0 && (
        <Paper withBorder p="xl" mt="md" radius="md" className="print:hidden">
          <Text ta="center" c="dimmed">
            No inspection readings logged yet for the selected Part, Operation,
            and Date. Try clicking "Generate Report" above.
          </Text>
        </Paper>
      )}
    </div>
