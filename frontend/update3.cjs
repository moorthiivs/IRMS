...
const renderNew = `                <Table.Tbody>
                  {Object.entries(groupedRecent).map(([l1Key, l1Node]: [string, any]) => (
                    <React.Fragment key={l1Key}>
                      {/* LEVEL 1 */}
                      <Table.Tr 
                        className={\`font-bold cursor-pointer transition-colors border-l-[4px] border-l-blue-600 \${l1Node.txs.some((tx: any) => tx.status === 'REJECTED') ? 'bg-orange-50 hover:bg-orange-100 dark:bg-orange-900/20 dark:hover:bg-orange-900/30 text-orange-900 dark:text-orange-200 border-b border-orange-200 dark:border-orange-900/50' : 'bg-gray-100 hover:bg-gray-200 dark:bg-[#1a1b1e] dark:hover:bg-[#25262b] border-b-2 border-gray-300 dark:border-gray-700 shadow-sm'}\`}
                        onClick={() => toggleGroup(l1Key)}
                      >
                        <Table.Td colSpan={8} className="py-3">
                          <Group justify="space-between">
                            <Group gap="sm">
                              <ThemeIcon variant="filled" color={expandedGroups[l1Key] ? "blue" : "gray"} size="sm" radius="xl">
                              ...
                              />
                              <Text size="sm" fw={800} className="text-gray-900 dark:text-gray-100">
                                {l1Key}
                              </Text>
                              <Badge size="xs" variant="filled" color="dark" radius="sm">{l1Node.txs.length} item{l1Node.txs.length > 1 ? 's' : ''}</Badge>
                            </Group>
                            <Group>
                      ...
                      {expandedGroups[l1Key] && Object.entries(l1Node.children).map(([l2Key, l2Node]: [string, any]) => {
                        ...
                        return (
                          <React.Fragment key={l2FullKey}>
                            {/* LEVEL 2 */}
                            <Table.Tr 
                              className="font-semibold cursor-pointer bg-slate-50 hover:bg-slate-100 dark:bg-[#25262b]/80 border-b border-gray-200 dark:border-gray-700 border-l-[4px] border-l-indigo-500"
                              onClick={() => toggleGroup(l2FullKey)}
                            >
                              <Table.Td colSpan={8} className="py-2 pl-10">
                                <Group justify="space-between">
                                  <Group gap="sm">
                                    <ThemeIcon variant="light" color={expandedGroups[l2FullKey] ? "indigo" : "gray"} size="xs" radius="xl">
                                    ...
                                    <Badge size="xs" variant="light" color="indigo">{l2Node.txs.length} items</Badge>
                                  </Group>
                                </Group>
                              </Table.Td>
                            </Table.Tr>
                            ...
                            {expandedGroups[l2FullKey] && Object.entries(l2Node.children).map(([l3Key, l3Node]: [string, any]) => {
                              ...
                              return (
                                <React.Fragment key={l3FullKey}>
                                  {/* LEVEL 3 */}
                                  <Table.Tr 
                                    className="font-medium cursor-pointer bg-white hover:bg-gray-50 dark:bg-[#2c2e33]/50 border-b border-gray-200 dark:border-gray-800/50 border-l-[4px] border-l-violet-400"
                                    onClick={() => toggleGroup(l3FullKey)}
                                  >
                                    <Table.Td colSpan={8} className="py-2 pl-16">
                                      <Group justify="space-between">
                                        <Group gap="sm">
                                          <ThemeIcon variant="light" color={expandedGroups[l3FullKey] ? "violet" : "gray"} size="xs" radius="xl">
                                          ...
                                          <Badge size="xs" variant="outline" color="violet">{l3Node.txs.length} items</Badge>
                                        </Group>
                                        <Group>
                                  ...
                                  {expandedGroups[l3FullKey] && l3Node.txs.map((item: any) => (
                                    <Table.Tr 
                                      key={item.id} 
                                      className={
                                        \`border-l-[4px] border-l-gray-300 \${selectedIds.includes(item.id) ? 'bg-red-50/40 dark:bg-red-900/20' 
